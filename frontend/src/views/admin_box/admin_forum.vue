<template>
    <div class="admin-forum">
        ...
        <el-table :data="paginatedData" style="width: 100%">
            <el-table-column prop="forum_id" label="forum_ID" width="120"></el-table-column>
            <el-table-column prop="forum_name" label="论坛名称" width="180"></el-table-column>
            <el-table-column prop="description" label="论坛描述" width="280">
                <template v-slot="scope">
                    <span>{{ formatDescription(scope.row.description) }}</span>
                </template>
            </el-table-column>
            <el-table-column prop="username" label="版主" width="150"></el-table-column>
            <el-table-column label="操作" width="240">
                <template v-slot="scope">
                    <el-button @click="transferForum(scope.row.forum_id)" type="warning" size="small">更换版主</el-button>
...
export default {
    ...
    methods: {
        ...
        async fetchForums() {
            try {
                const response = await axios.get('/admin/get_forums');  // 调用后端接口获取论坛数据
                this.forums = response.data;  // 假设接口返回的数据是论坛列表
            } catch (error) {
