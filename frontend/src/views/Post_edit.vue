    ...
    <div class="post-edit">
        ...
        <div v-if="!isEditing" class="forum-select">
            <el-select v-model="selectedForum" placeholder="请选择论坛" clearable class="select-forum">
                <el-option v-for="forum in forums" :key="forum.forum_id" :label="forum.forum_name" :value="forum.forum_id"></el-option>
            </el-select>
        </div>
...
export default {
    ...
    setup() {
        ...
        const loadForums = async () => {
            try {
                const { data } = await axios.get("/get_forums"); // 假设后端提供的接口
                forums.value = data;
            } catch (error) {
        ...
        const handleSubmit = async () => {
            ...
            try {
                if (isEditing.value) {
                    axios.post(`/post${postId}/edit_post`, {
                    ...
                    axios.post("/forum"+selectedForum.value+"/release_post", {
                        title: postTitle.value,
                        body: postContent.value,
                    }).then((res) => {
                    console.log(res.data.message);
